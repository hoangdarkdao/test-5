...
class MOMCTS_AHD:
    def __init__(self,
        ...
        self.current_metric_focus = FOCUS_AREA[2] # default mode
        ...
        if option == 's1':
            ...
            while i < 3:
                prompt = MOMCTSPrompt.get_prompt_s1(
                    self._task_description_str, path_set, self._function_to_evolve, self.current_metric_focus)
            ...
            prompt = MOMCTSPrompt.get_prompt_e1(
                self._task_description_str, indivs, self._function_to_evolve, self.current_metric_focus)
            func = self._sample_evaluate_register(prompt, func_only=True)
            if func is False:
                is_valid_func = False
            else:
            ...
            while i < 3:
                ...
                while True:
                    ...
                    if now_indiv != cur_node.individual:
                        break
                prompt = MOMCTSPrompt.get_prompt_e2(self._task_description_str, [now_indiv, cur_node.individual],
                                                self._function_to_evolve, self.current_metric_focus)
                func = self._sample_evaluate_register(prompt, func_only=True)
                if func is False:
                    is_valid_func = False
                    i += 1
    ...
    def run(self):
        ...
        while self._continue_loop():  # if current evaluation < max evaluation, still evaluate function
            i = self._population._generation % 3
            self.current_metric_focus = FOCUS_AREA[i]
            print(f"Current metric focus: {self.current_metric_focus}")
            node_set = []
            print(
            ...
            self._population.survival() # _generation += 1
